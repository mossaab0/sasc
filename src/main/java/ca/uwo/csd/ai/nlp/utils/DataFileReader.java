...
public class DataFileReader {
    ...
    public static Instance readLine(String line) {
        String[] tokens = line.split("\\s+");
        SparseVector vector = new SparseVector(tokens.length - 1);
        for (int i = 1; i < tokens.length; i++) {
            String[] fields = tokens[i].split(":");
            vector.add(Integer.parseInt(fields[0]), Double.parseDouble(fields[1]));
        }
        return new Instance(Double.parseDouble(tokens[0]), vector);
    }
